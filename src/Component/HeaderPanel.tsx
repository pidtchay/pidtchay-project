...
export const HeaderPanel:React.FC<IHeaderPanel> = ({isSideMenuToogle, onToggleTrueFalse}) => {
    ...
    const changeLanguage = () => {
        setLanguage(language === ESupportedLanguages.EN
            ...
            <Row justify="space-between">
                ...
                <Col span={5}>
                    <div className={style.header_layout}>
                        <Button type="primary" onClick={changeLanguage} className={style.header_layout_i18n} >{languageUsed}</Button>
                        {auth && <Avatar size="large" icon={<UserOutlined />} className={style.header_layout_avatar} />}
                        {auth && <Button type="primary" shape="round" icon={<LoginOutlined />} size="large" className={style.header_layout_button} onClick={() => dispatch(cleanSession())}>{get(literals, 'ACTIONS.logout')}</Button>}
                    </div>
                </Col>
            </Row>
        </div>
